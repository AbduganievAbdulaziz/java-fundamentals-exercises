    ...
    static class CollectionUtil {
        ...
        public static<T extends BaseEntity> boolean hasDuplicates(Collection<T> entities, T targetEntity) {
            ...
            while(it.hasNext() && count < 2) {
                T entity = it.next();
                if (entity.getUuid().compareTo(targetEntity.getUuid()) == 0)
                    count++;
            }
            return count > 1;
        }
        ...
        public static<T>  Optional<T> findMax(Iterable<T> elements, Comparator<T> comparator) {
            ...
            while(it.hasNext()) {
                ...
                if (maxValue == null || comparator.compare(element, maxValue) > 0) {
                    maxValue = element;
                }
            }
        ...
        public static<T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            ...
            Optional<T> result = findMax((Iterable) entities, CREATED_ON_COMPARATOR);
        ...
        public static void swap(List<?> elements, int i, int j) {
            ...
            makeSwap(elements, i, j);
        }

        private static<T> void makeSwap(List<T> elements, int i, int j) {
